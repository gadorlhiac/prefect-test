...
def sub_overridable_arguments(
        ...
        try:
            ...
            if ntasks > max_cores:
                slurm_param_str = re.sub(
                    pattern, f"{max_cores}", slurm_param_str
                )
    ...
    if max_nodes is not None:
        ...
        try:
            ...
            slurm_param_str = re.sub(
                pattern, f"nodes=0-{max_nodes}", slurm_param_str
            )
    ...
    if require_partition is not None:
        ...
        try:
            ...
            slurm_param_str = (
                f"{slurm_param_str} --partition={require_partition}"
            )
...
def create_control_doc(
    ...
    if lute_params["debug"]:
        lute_param_str = (
            f"--taskname {lute_task_id} --config {config_path} --debug"
        )
    else:
    ...
    if custom_slurm_params:  # SLURM params != ""
        ...
        slurm_param_str = sub_overridable_arguments(slurm_param_str, max_cores, max_nodes, require_partition)
...
def parse_response(
    ...
    if resp.status_code not in (200,):
        raise Exception(f"Bad response from JID {resp}: {resp.content}")
    try:
